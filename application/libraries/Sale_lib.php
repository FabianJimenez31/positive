...
class Sale_lib
{
	...
	*/
	...
	function add_item($item_id,$quantity=1,$discount=0,$price=null,$regular_price = null, $cost_price = null, $description=null,$serialnumber=null, $force_add_or_update = FALSE, $line = FALSE, $update_register_cart_data = TRUE,$apply_price_rules = TRUE,$rule = array(),$osT_return_line = FALSE)
	{	
    ...
    $itemalreadyinsale=FALSE;        //We did not find the item yet.
  	...
  	$item_info = $this->CI->Item->get_info($item_id);		
		...
		osticketfieldsShowd(array(
			...
			'is_osticket'=>$item_info->is_osticket,
			'special_force_serial'=>$item_info->special_force_serial,
			'is_service'=>$item_info->is_service,
			'osticket_fields'=>$item_info->osticket_fields,
			'quantity'=>$quantity,
      'discount'=>$discount,
	...
	function edit_item($line,$description = NULL,$serialnumber = NULL,$quantity = NULL,$discount = NULL,$price = NULL, $cost_price = NULL,$apply_price_rules = TRUE, $rule = NULL , $ostValues=false)
	{		
		$items = $this->get_cart();
		...
		if(isset($items[$line]))
		{
			...
			if($items[$line]['is_service']==1 && $items[$line]['is_osticket']==1){

				$osT_Fields=json_decode($items[$line]['osticket_fields'],1);
				foreach($osT_Fields as $ostf){

					if(isset($_POST['name']) && $_POST['name']=='osf_'.$ostf['field']){
						$items[$line]['osf_'.$ostf['field']]=$_POST['value'];
					}

				}
			
			}
			if($ostValues!=false && is_array($ostValues) && isset($ostValues['name']) && isset($ostValues['value'])){
				if($items[$line]['is_service']==1 && $items[$line]['is_osticket']==1){

					if($ostValues['name']=='osTPreloaded'){

						$items[$line]['__osTPreload__']=$ostValues['value'];

					}elseif($ostValues['name']=='osTPreloadedEnded'){

						$items[$line]['__osTPreloadX__']=$ostValues['value'];
					...
					foreach($osT_Fields as $ostf){
						...
						if($ostValues['name']==$ostf['field']){
							$items[$line]['osf_'.$ostf['field']]=$ostValues['value'];
						}
	...
	function copy_entire_sale($sale_id, $is_receipt = false)
	{
		...
		foreach($this->CI->Sale->get_sale_items($sale_id)->result_array() as $row)
		{
			...
			$llave=$this->add_item($row['item_id'].'|FORCE_ITEM_ID|',$row['quantity_purchased'],$row['discount_percent'],$price_to_use,$row['regular_item_unit_price_at_time_of_sale'], $row['item_cost_price'], $row['description'],$row['serialnumber'], TRUE, $row['line'], FALSE, FALSE,$rule,TRUE);

			if($item_info->is_service && $item_info->is_osticket){

				$osFields=json_decode($item_info->osticket_fields,1);
				$osPreload=json_decode($row['osticket_sent'],1);

				$this->edit_item($llave,NULL,NULL,NULL,NULL,NULL, NULL,TRUE, NULL , array('name'=>'osTPreloaded','value'=>$row['osticket_sent']));
				$this->edit_item($llave,NULL,NULL,NULL,NULL,NULL, NULL,TRUE, NULL , array('name'=>'osTPreloadedEnded','value'=>$row['osticket_number']));
				foreach($osFields as $osff){
					if($osff['value']=='manual.input'){
						$this->edit_item($llave,NULL,NULL,NULL,NULL,NULL, NULL,TRUE, NULL , array('name'=>$osff['field'],'value'=>$osPreload[$osff['field']]));
					}
