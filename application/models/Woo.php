...
class Woo extends  Ecom
{
	...
	function sync_inventory_changes()
	{
		...
		while($send_call == true) 
		{
			...
			$this->db->where_in('product_id', $woo_product_ids);
			$all_ecom_products_batch_result = $this->db->get()->result_array();
			foreach($all_ecom_products_batch_result as $row)
			{
				$all_ecom_products_batch[$row['product_id']] = $row;
			}
			
			foreach($result_products as $woo_product)
			{
				if (isset($all_ecom_products_batch[$woo_product['id']])) 
				{
					$item = $all_ecom_products_batch[$woo_product['id']];
					...
					else
					{
						...
						$prev_quantity=   $item['product_quantity'];
