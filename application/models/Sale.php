...
class Sale extends CI_Model
{
	...
	function get_cash_sales_total_for_shift($shift_start, $shift_end)
  {
	...
	function _get_all_sale_payments($sale_ids)
	{
		...
		if (count($sale_ids) > 0)
		{
      	...
      	$this->db->join('sales', 'sales.sale_id=sales_payments.sale_id');
			...
			$this->db->order_by("(".$this->db->dbprefix("sales_payments").".payment_type IN ($points_payment_types)) DESC, (SUBSTRING_INDEX(".$this->db->dbprefix("sales_payments").".payment_type,':',1) IN ($giftcard_payment_types)) DESC,"."(".$this->db->dbprefix("sales_payments").".payment_type IN ($cash_payment_types)) DESC,payment_date");
	...
	function save($items,$customer_id,$employee_id, $sold_by_employee_id, $comment,$discount_reason,$show_comment_on_receipt, $coupons, $payments,$sale_id=false, $suspended = 0, $change_sale_date=false,$balance=0, $store_account_payment = 0)
	{
		...
		$cliO=(array) $this->Customer->get_info($customer_id);
		$empleado_id=($sold_by_employee_id>1)?$sold_by_employee_id:$employee_id;
		$empO=(array) $this->Employee->get_info($empleado_id);
		...
		{			
			...
			if (isset($item['item_id']))
			{
				...
				if($item['is_service']==1 && $item['is_osticket']==1){
					$this->load->helper('osticket');
					$osCampos=json_decode($item['osticket_fields'],1);					
					$enviar=array();
					$detalles_venta=lang('common_item_number').": ".$item['item_number']." \n".lang('common_product_id').": ".$item['product_id']." \n".lang('common_item_name').": ".$item['name']." \n".lang('common_description').": ".$item['description'];
					

					foreach($osCampos as $osCamp){

						if(!isset($enviar[$osCamp['field']])){

							if(strpos($osCamp['value'],'manual') !== false){

								$enviar[$osCamp['field']]=$item['osf_'.$osCamp['field']];
								$detalles_venta.=" \n".$osCamp['label'].": ".$item['osf_'.$osCamp['field']];
							}elseif(strpos($osCamp['value'],'customer') !== false){
								if($osCamp['value']=='customer.phone'){
									$enviar[$osCamp['field']]=$cliO['phone_number'];
									$detalles_venta.=" \n".$osCamp['label'].": ".$cliO['phone_number'];

								}elseif($osCamp['value']!='customer.name'){
									$enviar[$osCamp['field']]=$cliO[(str_replace('customer.','',$osCamp['value']))];
									$detalles_venta.=" \n".$osCamp['label'].": ".$cliO[(str_replace('customer.','',$osCamp['value']))];
								}else{
									$enviar[$osCamp['field']]=$cliO['first_name']." ".$cliO['last_name'];
									$detalles_venta.=" \n".$osCamp['label'].": ".$cliO['first_name']." ".$cliO['last_name'];
								}
							}elseif(strpos($osCamp['value'],'employee') !== false){

								if($osCamp['value']=='employee.phone'){
									$enviar[$osCamp['field']]=$empO['phone_number'];
									$detalles_venta.=" \n".$osCamp['label'].": ".$empO['phone_number'];

								}elseif($osCamp['value']!='employee.name'){
									$enviar[$osCamp['field']]=$empO[(str_replace('employee.','',$osCamp['value']))];
									$detalles_venta.=" \n".$osCamp['label'].": ".$empO[(str_replace('employee.','',$osCamp['value']))];
								}else{
									$enviar[$osCamp['field']]=$empO['first_name']." ".$empO['last_name'];
									$detalles_venta.=" \n".$osCamp['label'].": ".$empO['first_name']." ".$empO['last_name'];
								}
							}
						}
					...
					$sales_items_data['is_osticket']=1;
					$sales_items_data['osticket_fields']=$item['osticket_fields'];
					$sales_items_data['osticket_sent']=json_encode($enviar);
					$enviar['_osT_sale_details']=$detalles_venta;
					$osticketNewNumber=osticketCreateTicket($this->config->item('osTicketsHost'),$this->config->item('osTicketsAPI'),$enviar);

					$sales_items_data['osticket_number']=($osticketNewNumber!=false)?$osticketNewNumber:'ERROR';
				...
				if ($suspended != 2)
				{
					...
					{			
						...
						$this->email->from($this->Location->get_info_for_key('email') ? $this->Location->get_info_for_key('email') : 'no-reply@mercurio.skyone.com.co', $this->config->item('company'));
