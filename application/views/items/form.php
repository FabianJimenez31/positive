	...
	<div class="panel panel-piluku">
		<div class="panel-heading">
			...
			<div class="panel-body">
				...
				<div class="form-group <?php if ($item_info->is_service){echo '';}else{echo'hidden';} ?>" id="show_osTicket">
					<?php echo form_label("osTicket".':', 'is_osticket',array('class'=>'col-sm-3 col-md-3 col-lg-2 control-label wide')); ?>
					<div class="col-sm-9 col-md-9 col-lg-10">
						<?php echo form_checkbox(array(
							'name'=>'is_osticket',
							'id'=>'is_osticket',
								'class'=>'delete-checkbox',
							'value'=>1,
							'checked'=>($item_info->is_osticket || (!$item_info->item_id && $this->config->item('default_new_items_to_service'))) ? 1 : 0)
						);?>
						<label for="is_osticket"><span></span></label>
					</div>
				</div>

				<div id="show_osTicketoptions" class="<?php if ($item_info->is_osticket){echo '';}else{echo'hidden';} ?>">

					<input type="hidden" id="osticketjson" name="osticket_fields"/>


						<div class="col-md-12">

							<div class="col-md-3"><b><?php echo lang('common_tag'); ?></b></div>
							<div class="col-md-3"><b><?php echo lang('common_giftcards_card_value'); ?></b></div>
							<div class="col-md-3"><b><?php echo lang('common_fields'); ?></b></div>
							<div class="col-md-1"><b><?php echo lang('common_required'); ?></b></div>
							<div class="col-md-1"><b></b></div>
							<div class="col-md-1"><b><a class="btn btn-success" onclick="javascript: newosT();"><i class="ion ion-android-add"></i></a></b></div>


						</div>

						<div id="osticketFields" class="col-md-12">
						</div>
						<hr/>
				</div>
...
var osticketFields = [];
var osticketID = 1;
var osticketBase = <?php  

	$osTbase=array();
	$osTest=$item_info->osticket_fields;
	$osTcomp=json_decode($osTest,1);
	if(count($osTcomp)>=1 && is_array($osTcomp[0])){

		$osTbase=$osTcomp;

	}

	echo json_encode($osTbase);

 ?> ;

var osticketOptions = <?php echo json_encode(osticketvalueslabels()); ?>;

var osticketCleanValues = <?php echo json_encode(osticketinitvalues(array())); ?>;

var osticketInitValues = <?php echo json_encode(osticketinitvalues($osTbase)); ?>;
...
function initosT(){
	...
	osticketInitValues.map(function(campo){
		campo.id=osticketID;
		osticketFields.push(campo);
		osticketID++ ;
...
function cleanosT(){
	...
	osticketCleanValues.map(function(campo){
		campo.id=osticketID;
		osticketFields.push(campo);
		osticketID++ ;
...
function osTupdate(indice,campo,valor){

	osticketFields=osticketFields.map(function(elemento){
...
function osTdelete(indice){

	osticketFields=osticketFields.filter(function(elemento){
...
function redrawosT(){
	...
	osticketFields=osticketFields.map(function(campo){
		separador = $('<hr>');
		contenedor = $('<div>');
		contenedor.addClass('col-md-12');
		contenedor.append(separador);

		nombre_campo = $('<div>');
		nombre_campo.addClass('col-md-3');

		nombre_campo_input = $('<input>');
		nombre_campo_input.addClass('form-control');
		nombre_campo_input.addClass('form-inps');
		nombre_campo_input.data('indice',campo.id);
		nombre_campo_input.val(campo.label);		
		nombre_campo_input.on('change',function(){

			osTupdate($(this).data('indice') , 'label',$(this).val());

		});

		nombre_campo.append(nombre_campo_input);
		contenedor.append(nombre_campo);

		valores_campo = $('<div>');
		valores_campo.addClass('col-md-3');
		valores_temp_options='';

		if(campo.value==''){

			campo.value='customer.account_number';

		}

		osticketOptions.map(function(valores){

			seleccionado=(campo.value == valores.option)?' selected ':'';

				valores_temp_options += "<option value='" + valores.option + "' " + seleccionado + " >" + valores.display + "</option>";

		});


		valores_campo_input = $('<select>');
		valores_campo_input.append(valores_temp_options);
		valores_campo_input.addClass('form-control');
		valores_campo_input.addClass('form-inps');
		valores_campo_input.data('indice',campo.id);
		valores_campo_input.on('change',function(){

			osTupdate($(this).data('indice') , 'value',$(this).val());

		});
		valores_campo.append(valores_campo_input);
		contenedor.append(valores_campo);


		tag_campo = $('<div>');
		tag_campo.addClass('col-md-3');

		tag_campo_input = $('<input>');
		tag_campo_input.addClass('form-control');
		tag_campo_input.addClass('form-inps');
		tag_campo_input.data('indice',campo.id);
		tag_campo_input.val(campo.field);
		tag_campo_input.on('change',function(){

			osTupdate($(this).data('indice') , 'field',$(this).val());

		});

		tag_campo.append(tag_campo_input);
		contenedor.append(tag_campo);

		if(campo.editable!=true){
			tag_campo_input.attr("disabled", true);
		}

		required_campo = $('<div>');
		required_campo.addClass('col-md-1');
		required_campo_anchor = $('<a>');
		required_campo_anchor.addClass('btn btn-info');
		required_icon = $("<i>");
		required_icon_class=(campo.required==true)?'ion-android-checkbox-outline':'ion-android-checkbox-outline-blank';
		required_icon.addClass('ion');
		required_icon.addClass(required_icon_class);

		if(campo.change_required==true){

			valor_required = (campo.required==true)?false:true;
			required_campo_anchor.data('indice',campo.id);
			required_campo_anchor.data('valor',valor_required);
			required_campo_anchor.on('click',function(){

				osTupdate($(this).data('indice') , 'required',$(this).data('valor'));
		...
		required_campo_anchor.append(required_icon);
		required_campo.append(required_campo_anchor);
		contenedor.append(required_campo);


		hide_campo = $('<div>');
		hide_campo.addClass('col-md-1');
		if(campo.only_hide==true){


			hide_campo_anchor = $('<a>');
			hide_campo_anchor.addClass('btn btn-info');
			hide_icon = $("<i>");
			hide_icon_class=(campo.hide==true)?'ion-eye-disabled':'ion-eye';
			hide_icon.addClass('ion');
			hide_icon.addClass(hide_icon_class);

			valor_hide = (campo.hide==true)?false:true;
			hide_campo_anchor.data('indice',campo.id);
			hide_campo_anchor.data('valor',valor_hide);
			hide_campo_anchor.on('click',function(){

				osTupdate($(this).data('indice') , 'hide',valor_hide,$(this).data('valor'));

			});


		
			hide_campo_anchor.append(hide_icon);
			hide_campo.append(hide_campo_anchor);



		}
		
		contenedor.append(hide_campo);


		delete_campo = $('<div>');
		delete_campo.addClass('col-md-1');
		if(campo.can_delete==true){


			delete_campo_anchor = $('<a>');
			delete_campo_anchor.addClass('btn btn-danger');
			delete_icon = $("<i>");
			delete_icon_class='ion-trash-a';
			delete_icon.addClass('ion');
			delete_icon.addClass(delete_icon_class);

			delete_campo_anchor.data('indice',campo.id);

			delete_campo_anchor.on('click',function(){

				osTdelete( $(this).data('indice') );

			});


		
			delete_campo_anchor.append(delete_icon);
			delete_campo.append(delete_campo_anchor);



		}
		
		contenedor.append(delete_campo);





		$("#osticketFields").append(contenedor);
		return campo;
	});

	$("#osticketjson").val(JSON.stringify(osticketFields));
  ...
  setTimeout(function(){$(":input:visible:first","#item_form").focus();},100);
	...
	$("#is_service").change(function()
	{
		if ($(this).prop('checked'))
		{
			...
			$("#show_osTicket").removeClass('hidden');
			$(".quantity-input").addClass('hidden');			
			$(".reorder-input").addClass('hidden');			
		}
		else
		{
			$("#show_osTicket").addClass('hidden');
			$(".quantity-input").removeClass('hidden');
			$(".reorder-input").removeClass('hidden');
		}
	});

	$("#is_osticket").change(function()
	{
		if ($(this).prop('checked'))
		{
			
			$("#show_osTicketoptions").removeClass('hidden');
		
		}
		else
		{
			$("#show_osTicketoptions").addClass('hidden');
...
$('#grid-loader').hide();
		...
		validator.resetForm();
		cleanosT();
		setTimeout(function(){ cleanosT(); }, 2000);
