...
class Config extends Secure_area 
{
	...
	function save()
	{
      ...
      curl_close($ch);		
		...
		'osTicketsHost'=>$this->input->post('osTicketsHost'),
		'osTicketsAPI'=>$this->input->post('osTicketsAPI'),
		...
		if($this->input->post('item_id_auto_increment'))
		{
			...
			if(!$auto_increment)
			{
				echo json_encode(array('success'=>false,'message'=>lang('config_change_auto_increment_item_id')));
				exit;
			} else {
		...
		if($this->input->post('item_kit_id_auto_increment'))
		{
			...
			if(!$auto_increment)
			{
				echo json_encode(array('success'=>false,'message'=>lang('config_change_auto_increment_item_kit_id')));
				exit;
			} else {
		...
		if($this->input->post('sale_id_auto_increment'))
		{
			...
			if(!$auto_increment)
			{
				echo json_encode(array('success'=>false,'message'=>lang('config_change_auto_increment_sale_id')));
				exit;
			} else {
		...
		if($this->input->post('receiving_id_auto_increment'))
		{
			...
			if(!$auto_increment)
			{
				echo json_encode(array('success'=>false,'message'=>lang('config_change_auto_increment_receiving_id')));
				exit;
			} else {
		...
		if (is_on_demo_host())
		{
			...
			$batch_save_data['company_logo'] = 0;
			$batch_save_data['company'] = 'Mercurio, Inc';
