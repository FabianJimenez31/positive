...
class Locations extends Secure_area implements Idata_controller
{
	...
	*/
	...
	function view($location_id=-1,$redirect=false)
	{
		...
		/** Este Renglón Deshabilita pedir nueva licencia **/
		$data['needs_auth'] = FALSE;
	...
	function does_have_valid_number_of_locations_for_an_additional_location($email)
	{
		...
		/**************************************
MODIFICAR LO SIGUIENTE PARA LICENCIA
			
			$current_location_count = $this->Location->count_all();
			$auth_url = (!defined("ENVIRONMENT") or ENVIRONMENT == 'development') ? 'http://mercvaldev.skyone.com.co/allowed_stores.php?email='.rawurlencode($email): 'http://mercval.skyone.com.co/allowed_stores.php?email='.rawurlencode($email);
	...
	function save($location_id=-1)
	{
		...
		if ($location_id == -1)
		{
			...
			$this->load->helper('demo');
			/****** AQUI DESHABILITAMOS EL CHEQUEO DE LICENCIA ***/
		...
		{
			echo json_encode(array('success'=>false,'message'=>lang('locations_error_adding_updating')."AQUI"));
		}
